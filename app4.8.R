...
injuries <- read_delim("neiss/injuries.tsv", 
                       delim = "\t", escape_double = FALSE, 
                       trim_ws = TRUE)
products <- read.delim2("~/ShinyTutorial/neiss/products2.tsv")
...
ui <- fluidPage(
  fluidRow(
    ...
    column(2, numericInput("n_rows", "Show number of rows:", 5,
                           min =1, max = 100))
  ),  
  fluidRow(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$code, {
    updateNumericInput(session, "n_rows", max = max_no_rows())
  })
  
  table_rows <- reactive(input$n_rows - 1)
